...
# === REGIME FROM RATIO (empirically perfect) ===
...
# === SIMULATION WITH BLACK SWAN + CLUSTERING ===
    ...
    with st.spinner("500 paths with real-world nasties..."):
        ...
        for _ in range(500):
            ...
            for _ in range(1200):
                ...
                # Black swan ~once every 18–24 months
        ...
        with col1:
            ...
            fig.update_layout(template="plotly_dark", title="Final Wealth (millions)", height=500)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            st.metric("Median Outcome", f"${np.median(finals)/1e6:.2f}M")
            st.metric("95th Percentile", f"${np.percentile(finals,95)/1e6:.2f}M")
            st.metric("Avg Simulated CAGR", f"{np.mean(sim_cagr):.1%}")
            st.metric("Ruin Rate (<$10k)", f"{(finals<10000).mean():.1%}")

st.markdown("<p style='text-align:center;color:#64748b;margin-top:100px;'>v5 PRO — ONE LIVE NUMBER • REAL SWANS • REAL STREAKS • ZERO BULLSHIT • 2025</p>", unsafe_allow_html=True)
